...
const LineChart = ({ width, height, margin = defaultMargin, data }) => {
    ...
    const yScale = useMemo(() =>
        scaleLinear({
            range: [yMax, 40],
            round: true,
            domain: [Math.min(...data.map(getSubnum)), Math.max(...data.map(getSubnum))],
        })
    );
                ...
                <div>
                    ...
                    >
                        ...
                        <div className='tooltip-value'>
                            <strong>{tooltipData.subnum}</strong>
                        </div>
                    </TooltipInPortal>
                </div>
            )}
...
const MonthlySubsNumberDataLineChart = (props) => {
    ...
    useEffect(() => {
        ...
        const getSubsNumberLine = async () => {
            const response = await getApi.post('/table_line_MonthlySubsNumberData', data);
            setSubsNumber(response.data);
        };
        getSubsNumberLine();
    }, []);
    if (!subsNumber) return null;
    return (
        <ParentSize
            style={{
                width: '100%',
        ...
        >
            {({ width, height }) => (
                <LineChart width={width} height={height} data={subsNumber} />
            )}
        </ParentSize>
    );
};

export default MonthlySubsNumberDataLineChart;
