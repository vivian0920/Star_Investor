...
const ProfitBarChart = (props) => {
    ...
    // console.log('ssscs', userid, id);
    useEffect(() => {
        var data = {
            userid: userid,
            shareID: id,
        };
        const getProfitBar = async () => {
            const response = await getApi.post('/table_bar_MonthlyRevenue', data);
    ...
    return (
        ...
        >
            {({ width, height }) => (
                <BarGraph width={width} height={height} data={profit} />
            )}
        </ParentSize>
    );
};
